...
import { createClient, updateClient } from "@/services/api/client";
import { getApiErrorMessage } from "@/utils/GetApiError";
import { clientdescriptions } from "../../page";
import { useParams, useSearchParams } from "next/navigation";
import { useOneClient } from "@/hooks/useOneClient";
...
const UpdateClient = () => {
  ...
  const { client, loading, error } = useOneClient(Number(params.id));

  useEffect(() => {
    setValue("nom", client?.nom ?? "");
    setValue("prenoms", client?.prenom ?? "");
    setValue("sexe", client?.sexe ?? "");
    setValue("email", client?.email ?? "");
    setValue("tel", client?.tel ?? "");
    setValue("nationalite", client?.nationalite ?? "");
    setValue("adresse", client?.adresse ?? "");
    setFocus({
      nom: Boolean(client?.nom),
      prenoms: Boolean(client?.prenom),
      sexe: Boolean(client?.sexe),
      email: Boolean(client?.email),
      tel: Boolean(client?.tel),
      nationalite: Boolean(client?.nationalite),
      adresse: Boolean(client?.adresse),
    });
  }, [client]);
    ...
    <div>
      ...
      >
        ...
        <p className="text-[10px] text-muted-foreground">
          Remplacez les informations à modifier
        </p>
